...
import { ArrowLeft, Droplets, Moon, Loader2, Footprints, Activity } from "lucide-react";
...
const ActivityItem = React.memo(({ type, config, state, onChange }: {
    type: ActivityType,
    config: any,
    state: { completed: boolean, value: string },
    onChange: (type: ActivityType, field: string, val: any) => void
}) => {
    const Icon = config.icon;
    return (
        <div className={cn(
            "flex flex-col gap-2 p-3 rounded-2xl border transition-colors duration-200", // Removed duration-300 and all for better perf
            state.completed ? "bg-primary/5 border-primary shadow-sm" : "bg-card border-border/50 shadow-sm" // Reduced shadow complexity
        )}>
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input
                        ...
                        onChange={(e) => onChange(type, 'completed', e.target.checked)}
                        className="h-5 w-5 rounded-md border-muted bg-background checked:bg-primary transition-colors cursor-pointer"
                    />
                </div>
            </div>
    ...
    return prev.state.completed === next.state.completed && prev.state.value === next.state.value;
});

ActivityItem.displayName = "ActivityItem";

const DebouncedNoteInput = React.memo(({ value, onChange }: { value: string, onChange: (val: string) => void }) => {
    ...
    return (
        <Input
            ...
            value={localValue}
            onChange={(e) => setLocalValue(sanitizeInput(e.target.value))}
...
function LogActivityForm() {
    ...
    const [activities, setActivities] = useState<Record<ActivityType, { completed: boolean, value: string }>>(DEFAULT_ACTIVITIES);
    // Track initial state to detect changes
    const [initialActivities, setInitialActivities] = useState<Record<ActivityType, { completed: boolean, value: string }>>(DEFAULT_ACTIVITIES);
    ...
    const fetchLogs = useCallback(async () => {
        ...
        try {
            ...
            if (result.success && result.activities.length > 0) {
                const newActivities = { ...DEFAULT_ACTIVITIES };
                let note = "";
                ...
                setInitialActivities(JSON.parse(JSON.stringify(DEFAULT_ACTIVITIES))); // Deep copy
                setCommonNote("");
            }
        } catch (error) {
    ...
    const handleActivityChange = useCallback((type: ActivityType, field: string, val: any) => {
        setActivities(prev => ({
            ...prev,
    ...
    return (
        <div className="min-h-screen bg-background pb-24">
            {/* Header - Removed backdrop-blur for performance */}
            <header className="px-4 py-4 flex items-center gap-4 border-b border-border/50 sticky top-0 bg-background/95 z-20">
            ...
            <main className="px-4 py-6 relative">
                {isLoading && (
                    <div className="absolute inset-0 bg-background/50 z-10 flex items-center justify-center min-h-[400px]">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/95 border-t border-border/50 z-30 sm:relative sm:bg-transparent sm:border-0 sm:p-0">
                        <Button
                            type="submit"
                            size="lg"
                            className="w-full text-lg h-14 rounded-2xl shadow-md shadow-primary/20 max-w-md mx-auto block"
